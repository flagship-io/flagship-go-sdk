...
func getMatchedVG(c *Campaign, visitorID string, context map[string]*structpb.Value) *VariationGroup {
	for _, vg := range c.VariationGroups {
		matched, err := commonDecision.TargetingMatch(vg.ToCommonStruct(), visitorID, context)
		if err != nil {
...
func (b *Engine) GetModifications(visitorID string, context map[string]*structpb.Value) (*model.APIClientResponse, error) {
	if b.getConfig() == nil {
